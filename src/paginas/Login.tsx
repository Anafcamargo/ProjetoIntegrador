...
const Login: React.FC = () => {
  ...
  const handleLoginUsuario = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // Aqui você armazena o token e o ID do usuário
      if (response.ok) {
        localStorage.setItem('token', data.token); // Armazena o token no localStorage
        console.log('Dados do usuário após login:', data); // Verifique a estrutura dos dados
        localStorage.setItem('userId', data.ID); // Armazena o ID do usuário no localStorage
        login(data.token, data.ID); // Chama a função de login do contexto
        navigate('/chamado');
      } else {
